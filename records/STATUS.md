...
- Highest-priority blocker: none; the public `1.2.0` DMG and Sparkle update are verified
...
- Date: 2026-09-26
  - Change: `1.2.0` shipped editable formatted mode (the native reader with in-place syntax reveal and the edit/formatted morph) and the list, quote, window, and session fixes, with the signed DMG and Sparkle update published publicly
  - Why it matters: existing users are offered formatted editing through Sparkle, replacing the read-only preview of `1.1.2`
...
- Date: 2026-09-11
  ...
  - Why it matters: the operator typed `- ` and could not delete it; lists were the last everyday block that could not be edited in place
  - Related ids: `DEC-20260910-001`

- Date: 2026-09-11
  - Change: heading commands work in formatted mode and animate (text crossfades, prefix fades, paragraphs below slide); a Paragraph command turns a heading back; empty lines are a full line tall without widening block gaps; fences and quote markers reveal around the cursor
  - Why it matters: the operator's second session in formatted mode reported snapping headings, a stub cursor on empty lines, and hidden fences
  - Related ids: `DEC-20260910-001`
...
- Date: 2026-09-10
  - Change: the text cursor crosses a mode switch with keyboard focus, revealed syntax morphs like other characters, and Command-B and Command-I work in formatted mode; the inline-code capsule no longer swallows the following space
  - Why it matters: switching modes mid-sentence and continuing to type now works in both directions
  - Related ids: `DEC-20260910-001`

- Date: 2026-09-10
  - Change: formatted mode accepts typing; every edit routes through the source map to the exact Markdown range via the editor's change path, syntax reveals around the caret, Enter starts a paragraph in prose, and composition commits once
  - Why it matters: the March feature is back without its failure mode; the displayed text never becomes the source
  - Related ids: `DEC-20260910-001`
...
- Date: 2026-09-09
  ...
  - Why it matters: this is the continuity the April research program set out to achieve, done with measured positions instead of an interpolating font
  - Related ids: `RSH-20260909-001`, `DEC-20260909-001`
...
- Next: accumulate and verify changes for the next release
  ...
  - Trigger: accepted product work changes the published `1.2.0` baseline
