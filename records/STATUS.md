...
- Current focus: publish and verify the requested `1.1.1` update-control repair
- Highest-priority blocker: none; the repair is implemented and release verification is in progress
- Next operator decision needed: none for `1.1.1`; its scope and timing are accepted
...
- Date: 2026-09-02
  - Change: the app-menu and Settings update controls now invoke the app-owned Sparkle controller directly
  - Why it matters: user-initiated update checks open Sparkle instead of silently returning through SwiftUI's internal application delegate
  - Related ids: `LOG-20260902-043709-260902`
...
- Next: publish and verify `1.1.1`
  - Owner: orchestrator plus the release workflow
  - Trigger: the update-control repair passes local verification
